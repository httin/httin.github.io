...
macOS ships its own old Ruby, so put the Homebrew one first on `PATH` in each
shell.
...
`_config.yml` is not reloaded by Jekyll, so restart the server after you change it.
